// The Game2 component is inspired by the NoPixel Untangle Minigame
// Source: https://sharkiller.ddns.net/nopixel_minigame/untangle/
...
const Game2 = () => {
  ...
  useEffect(() => {
    ...
    let amountOfDots = 5; // Increased the number of dots/nodes
    ...
    function createLines() {
      ...
      // Ensure that the lines are tangled, but limit the number of attempts
      let attemptsRemaining = 100; // Set a maximum number of attempts
      while (!lines.some(line => line.intersecting) && attemptsRemaining > 0) {
        const newLines = [];
        const newLimit = new Array(dots.length).fill(0);
    
        while (newLimit.some(num => num < 2)) {
          let from = random(0, dots.length);
          let to = random(0, dots.length);
          if (from === to) {
            continue;
          }
          if (newLimit[from] === max_connects || newLimit[to] === max_connects) {
            continue;
          }
          if (newLines.filter((el) => el.start === from && el.end === to).length > 0) {
            continue;
          }
          if (newLines.filter((el) => el.start === to && el.end === from).length > 0) {
            continue;
          }
          newLimit[from] += 1;
          newLimit[to] += 1;
        ...
        if (newLines.some(line => line.intersecting)) {
          lines = newLines;
          break;
        }
...
// Function to draw the lines
  ...
  // Return the JSX for the Game2 component
  return (
    <MDBCard
